    ...
    const renderSlotCard = (slot, index, compact = false) => {
        ...
        return (
            <div key={index} className={`${compact ? 'p-2' : 'p-3'} rounded-lg border border-slate-200 bg-white`}>
                <div className="grid gap-2 md:grid-cols-[190px_160px_minmax(0,1fr)_36px] md:items-start">
                    <div>
                        ...
                        <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-1.5">
                            <input
                                ...
                                className="min-w-0 cursor-pointer rounded-lg border border-slate-300 bg-white px-2 py-2 text-xs font-semibold text-slate-900 transition-all duration-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 disabled:cursor-not-allowed disabled:opacity-50"
                            />
       ...
       <div className={`${embedded ? 'p-2 sm:p-5 space-y-2 sm:space-y-4' : 'p-4 sm:p-6 lg:p-8 space-y-8'} max-w-5xl mx-auto bg-slate-50`}>
            ...
            <div className="animate-enter space-y-3 rounded-2xl border border-slate-200 bg-white p-2.5 shadow-sm sm:p-4" style={{ animationDelay: '80ms' }}>
                ...
                {getBreakSlots().map(({ slot, index }) => (
                    <section key={`break-${index}`} className="rounded-xl border border-slate-200 bg-slate-50 p-2.5 sm:p-4">
                        <div className="mb-3 flex flex-row gap-2 ">
                            <h2 className="text-sm font-bold text-slate-900 sm:text-base">Break: </h2>
                            <p className="text-xs mt-1 font-medium text-slate-500">{slot.slotType}</p>
                        </div>
                        {renderSlotCard(slot, index, embedded)}
                    </section>
                ))}
