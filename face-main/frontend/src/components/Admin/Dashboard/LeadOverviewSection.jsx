...
const LeadOverviewSection = () => {
  ...
  return (
    <div className="bg-white border border-slate-200/80 rounded-xl p-3.5 sm:p-5 shadow-sm space-y-3.5">
      {/* Section Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
        <div>
          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
            <h2 className="text-sm sm:text-base font-bold text-slate-900">Active Leads & Sales Progress</h2>
            <span className="text-[10px] sm:text-xs font-semibold px-2 py-0.5 rounded-full bg-indigo-50 text-indigo-700 border border-indigo-100 shrink-0">
              {activeLeads.length} Active
            </span>
          </div>
          <p className="text-slate-500 text-xs mt-0.5">Click any active lead to open and filter in Sales Dashboard</p>
        </div>

        <button
          onClick={handleViewAllSales}
          className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-700 hover:underline transition-all self-start sm:self-auto"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 bg-slate-50/80 p-2.5 rounded-lg border border-slate-100">
        <div className="flex items-center justify-between px-2 py-1.5 sm:py-1 bg-white rounded-md border border-slate-200/60 shadow-2xs">
          <span className="text-[11px] font-medium text-slate-500 truncate mr-1">New Leads</span>
          <span className="text-xs font-bold text-blue-600 shrink-0">{stageCounts.New}</span>
        </div>
        <div className="flex items-center justify-between px-2 py-1.5 sm:py-1 bg-white rounded-md border border-slate-200/60 shadow-2xs">
          <span className="text-[11px] font-medium text-slate-500 truncate mr-1">Contacted</span>
          <span className="text-xs font-bold text-amber-600 shrink-0">{stageCounts.Contacted}</span>
        </div>
        <div className="flex items-center justify-between px-2 py-1.5 sm:py-1 bg-white rounded-md border border-slate-200/60 shadow-2xs">
          <span className="text-[11px] font-medium text-slate-500 truncate mr-1">Proposal</span>
          <span className="text-xs font-bold text-purple-600 shrink-0">{stageCounts.Proposal}</span>
        </div>
        <div className="flex items-center justify-between px-2 py-1.5 sm:py-1 bg-white rounded-md border border-slate-200/60 shadow-2xs">
          <span className="text-[11px] font-medium text-slate-500 truncate mr-1">Est. Pipeline</span>
          <span className="text-xs font-bold text-emerald-600 shrink-0">₹{totalValue.toLocaleString()}</span>
        </div>
      </div>
            ...
            {displayLeads.map((lead) => {
              ...
              return (
                <div
                  ...
                  className="group relative flex flex-col justify-between p-3 rounded-lg border border-slate-200/80 bg-white hover:border-blue-300 hover:shadow-md transition-all duration-200 cursor-pointer"
                >
                  {/* Top Row: Name & Stage */}
                  <div className="flex items-start justify-between gap-2 mb-2">
                    ...
                    <span className={`shrink-0 text-[10px] font-semibold px-2 py-0.5 rounded-full border ${stageStyle}`}>
                      {lead.status || 'New'}
                    </span>
                  </div>

                  {/* Requirement / Notes summary */}
                  <p className="text-[11px] text-slate-600 line-clamp-2 mb-2.5 bg-slate-50 p-1.5 rounded border border-slate-100 leading-tight">
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1.5 border-t border-slate-100">
                    ...
                    {lead.estimatedValue ? (
                      ...
                      <span className="text-slate-400 shrink-0">N/A</span>
                    )}
                  </div>
          ...
          {activeLeads.length > INITIAL_COUNT && (
            <div className="flex justify-center pt-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200/80 active:bg-slate-200 rounded-lg transition-colors border border-slate-200 shadow-2xs"
              >
                {showAll ? (
                  <>
                    <span>Show Less</span>
